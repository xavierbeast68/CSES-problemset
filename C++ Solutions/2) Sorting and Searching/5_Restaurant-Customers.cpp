/**
 * CODER=> xavierbeast68
 * Problem : Restaurant Customers -> CSES Problem Set
 * Date : 19-12-2022
...
void brute_force(vector<pair<int, int>> v, int n)
{
    int maxCount = 0;
    FOR(i, 0, n - 1)
    {
...
void optimised_approach(vector<pair<int, int>> v)
{
...
void solve()
{
    ...
    {
        read(x); read(y);
        vec.push_back(make_pair(y, x));     // vec[finish_time, start_time]
    }
    sort(begin(vec), end(vec));     // sorted according to finish time
    ...
    */
    ...
    FOR(i, 0, n)
    {
        read(x);
        read(y);
        vec.pb(make_pair(x, 1));
        vec.pb(make_pair(y, -1));
    }
    sort(begin(vec), end(vec));
    optimised_approach(vec);
}
