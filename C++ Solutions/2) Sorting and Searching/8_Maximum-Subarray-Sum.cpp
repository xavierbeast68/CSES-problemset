/**
 * CODER=> xavierbeast68
 * Problem : Maximum Subarray Sum -> CSES Problem Set
 * Date : 19-12-2022
...
void solve()
{
    ...
    read(n);
    int currsum = 0, maxSum = LONG_LONG_MIN, smallest_nagtive = LONG_LONG_MIN;
    FOR(i, 0, n)
    {
        read(x);
        currsum += x;
        smallest_nagtive = max(smallest_nagtive, x);
        if (currsum < 0)
        {
            currsum = 0;
        }
    ...
    //! when all elements are negative -> kadane's fails
    // in that case print the smallest negative element present in the array.
    if (maxSum == 0)
    {
        maxSum = smallest_nagtive;
    }
    println(maxSum);
}
