 ...
 * Submission URL : https://cses.fi/paste/30c60f317ce1f2f25406ea/
...
// Brute Force
// int apple_division(vi arr, int i, int sack1, int sack2)
// {
//     if(i < 0)
//     {
...
//         {
//             sack1 += arr[i]; i--;
//             return (apple_division(arr, i , sack1, sack2));
//         }
//         else
//         {
//             sack2 += arr[i];i--;
...
int rec(vector<int> arr, int i, int a1, int a2)
{
    if(i<0)
    {
        return (abs(a1-a2));
    ...
    {
        int min1= rec(arr,i-1,a1+arr[i],a2);
        int min2= rec(arr,i-1,a1,a2+arr[i]);
...
void solve()
{
    ...
    read(n);
    vi arr = readvector(n);
    // Brute Force fails at many high value cases
    // sort(all(arr));
    // int ans = apple_division(arr, n-1, 0, 0);   // (array, iterator from back, sack1. sack2);
    // println(ans);
    
    
    // Optimized soln
    // trying all possible combinations and finding the best one
