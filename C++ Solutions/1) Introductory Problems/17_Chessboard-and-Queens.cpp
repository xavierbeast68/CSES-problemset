 ...
 * Submission URL : https://cses.fi/paste/39b0894226b3aa31585d8f/
...
void solveBoard(int currCol, int n, vector<vector<int>>& board){
    if(currCol == n){
        ans++;
        return;
    }
    ...
    for(int currRow = 0; currRow < n; currRow++){
        if(board[currRow][currCol] != -1 && check(board, currRow, currCol)){
            board[currRow][currCol] = 1;
            solveBoard(currCol+1, n, board);
