...
// Recursive Soln -> O(N^2) -> gives TLE
// void dfs(int n, vector<int>& arr, int ind, vector<int>& towers){
//     int sz = towers.size();
//     if(ind == n){
//         minTowers = min(minTowers, sz);
//         return;
//     }
...
//             int temp = towers[i];
//             towers[i] = arr[ind];
//             dfs(n, arr, ind+1, towers);
//             towers[i] = temp;
//             newTower = false;
//         }
//     }
//     if(newTower){
//         towers.push_back(arr[ind]);
//         dfs(n, arr, ind+1, towers);
//     }
// }

//* Using Multiset and Upper Bound-> O(NlogN)
void dfs(vi& arr, int n, int ind, multiset<int>& mt){
    if(ind == n){
        return;
    }

    // iterator to the upper bound of arr[ind]
    auto idx = mt.upper_bound(arr[ind]);
    if(idx == mt.end()){
        mt.insert(arr[ind]);
    }
    else{
